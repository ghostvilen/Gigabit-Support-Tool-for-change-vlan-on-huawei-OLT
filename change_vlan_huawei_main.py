...
def die(connect, message):
    ...
    connect.close()
    sys.exit(1)
# тут должна быть гора кода которая читает из тсв файла список олтов и заганяет его в сплывающий список и при выборе применяет в переменные ниже
host = #хост со списка тсв файла
new_Vlan = #влан с файла тсв сопоставимый олте
###########
